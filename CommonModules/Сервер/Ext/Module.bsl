...
Функция ЗапросВЯндекс(ТекстЗапроса) Экспорт
	...
	Если Результат.КодСостояния = 302 Тогда 
		...
		Если URI.Схема = "https" Тогда      
 			...
 			Ответ = БезопасноеСоединение.Получить(Новый HTTPЗапрос(URI.ПутьНаСервере, Заголовки)); 
	...
	Если ОтветЯндекса="" Тогда
		ОтветЯндекса="<html><body>" + "Ошибка: " + Ответ.КодСостояния + "</body></html>";
	КонецЕсли;
